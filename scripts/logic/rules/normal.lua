...
function PseudoregaliaNormalRules.new(cls, definition)
    ...
    local region_clauses = {
        ...
        ["Tower Remains -> The Great Door"] = function(state)
            return self:can_attack(state) and self:get_clings(state, 6) and self:kick_or_plunge(state, 1)
        end,
        ...
        ["Castle Main -> Castle => Theatre Pillar"] = function(state)
            return self:get_clings(state, 6) and self:kick_or_plunge(state, 1)
        ...
        ["Castle By Scythe Corridor -> Castle => Theatre (Front)"] = function(state)
            return self:get_clings(state, 6) and self:kick_or_plunge(state, 2)
        end,
    ...
    local location_clauses = {
        ...
        ["Dilapidated Dungeon - Dark Orbs"] = function(state)
            return self:get_clings(state, 6) and self:can_bounce(state)
        ...
        ["Castle Sansa - Alcove Near Scythe Corridor"] = function(state)
            return self:get_clings(state, 6) and self:get_kicks(state, 1) and self:has_plunge(state)
            or self:kick_or_plunge(state, 4)
        end,
        ...
        ["Listless Library - Time Trial"] = function (state)
            return self:has_breaker(state) and self:has_plunge(state) and self:get_clings(state, 6)
        end,
